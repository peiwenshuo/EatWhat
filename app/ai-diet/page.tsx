...
export default function AIDietPage() {
  ...
  useEffect(() => {
    const handleError = (event: ErrorEvent) => {
      console.error('[AI Diet Page] 全局错误:', {
    ...
    const handleUnhandledRejection = (event: PromiseRejectionEvent) => {
      console.error('[AI Diet Page] 未处理的 Promise 拒绝:', {
        reason: event.reason,
        userAgent: navigator.userAgent
      })
    }
    ...
    console.log('[AI Diet Page] 页面已加载', {
      userAgent: navigator.userAgent,
  ...
  useEffect(() => {
    if (status === 'unauthenticated') {
      console.log('[AI Diet Page] 未认证，重定向到登录页')
      router.push('/auth/signin')
    } else if (status === 'authenticated') {
      console.log('[AI Diet Page] 用户已认证:', session?.user?.email)
    }
